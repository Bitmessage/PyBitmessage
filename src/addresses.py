...
def decodeVarint(data):
    ...
    if firstByte == 253:
        ...
        if len(data) < 3:
            raise varintDecodeError(
                'The first byte of this varint as an integer is %s but the' +
                'total length is only %s. It needs to be at least 3.'
                % (firstByte, len(data)))
    ...
    if firstByte == 254:
        ...
        if len(data) < 5:
            raise varintDecodeError(
                'The first byte of this varint as an integer is %s but the' +
                'total length is only %s. It needs to be at least 5.'
                % (firstByte, len(data)))
    ...
    if firstByte == 255:
        ...
        if len(data) < 9:
            raise varintDecodeError(
                'The first byte of this varint as an integer is %s but' +
                'the total length is only %s. It needs to be at least 9.'
                % (firstByte, len(data)))
...
def encodeAddress(version, stream, ripe):
    if version >= 2 and version < 4:
        if len(ripe) != 20:
            raise Exception(
                "Programming error in encodeAddress: The length of a given" +
                "ripe hash was not 20.")
        if ripe[:2] == '\x00\x00':
            ripe = ripe[2:]
        elif ripe[:1] == '\x00':
            ripe = ripe[1:]
...
def decodeAddress(address):
    ...
    if addressVersionNumber == 1:
        ...
        if len(embeddedRipeData) == 19:
            return status, addressVersionNumber, streamNumber, '\x00'
            + embeddedRipeData
        ...
        if embeddedRipeData[0:1] == '\x00':
            '''In order to enforce address non-malleability, encoded RIPE data
            ...
            return status, addressVersionNumber, streamNumber, x00string
            +embeddedRipeData
...
if __name__ == "__main__":
    print 'Let us make an address from scratch. Suppose we generate two random'
    + '32 byte values and call the first one the signing key and the second'
    + 'one the encryption key:'
    privateSigningKey = '93d0b61371a54b53df143b954035d612f8efa8a3ed1cf842c2186'
    + 'bfd8f876665'
    privateEncryptionKey = '4b0b73a54e19b059dc274ab69df095fe699f43b17397bca26'
    + 'fdf40f4d7400a3a'
    print 'privateSigningKey =', privateSigningKey
    print 'privateEncryptionKey =', privateEncryptionKey
    print 'Now let us convert them to public keys by doing an elliptic curve'
    ...
    print 'Notice that they both begin with the \\x04 which specifies the'
    + 'encoding type. This prefix is not send over the wire. You must strip.'
    + 'if off before you send your public key across the wire, and you must'
    + 'add it back when you receive a public key.'
