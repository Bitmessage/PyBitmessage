# This program can be used to print(out
# everything in your Inbox or Sent folders
# and also take things out of the trash.
# Scroll down to the bottom to see the
# functions that you can uncomment. Save
# then run this file.
#
# The functions which only read the
# database file seem to function just fine
# even if you have Bitmessage running but
# you should definitly close it before
# running the functions that make changes
# (like taking items out of the trash).
...
def readInventory():
    ...
    output = cur.fetchall()
    for row in output:
        hsh, objecttype, streamnumber, payload, expirestime = row
        print('Hash:', hexlify(hsh), objecttype, streamnumber, '\t', hexlify(payload), '\t', unicode(strftime('%a, %d %b %Y  %I:%M %p', localtime(expirestime)), 'utf-8'))
