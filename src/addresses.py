...
def decodeBase58(string, alphabet=ALPHABET):
    ...
    try:
        ...
        # Char not found (like a space character or a 0)
        return 0
    return num
...
def decodeVarint(data):
    ...
    if firstByte == 253:
        ...
        if len(data) < 3:
            raise varintDecodeError('The first byte of this varint as an integer is %s but the total length is only %s. ' +
                                    'It needs to be at least 3.' % (firstByte, len(data)))
    ...
    if firstByte == 254:
        ...
        if len(data) < 5:
            raise varintDecodeError('The first byte of this varint as an integer is %s but the total length is only %s. ' +
                                    'It needs to be at least 5.' % (firstByte, len(data)))
    ...
    if firstByte == 255:
        ...
        if len(data) < 9:
            raise varintDecodeError('The first byte of this varint as an integer is %s but the total length is only %s. ' +
                                    'It needs to be at least 9.' % (firstByte, len(data)))
...
if __name__ == "__main__":
    print('Let us make an address from scratch. Suppose we generate two random 32 byte values and ' +
          'call the first one the signing key and the second one the encryption key:')
    ...
    print('Notice that they both begin with the \\x04 which specifies the encoding type. This prefix is not send over the wire. ' +
          'You must strip if off before you send your public key across the wire, and you must add it back when you receive a public key.')
