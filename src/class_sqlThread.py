    ...
    def run(self):
        ...
        try:
            ...
            if str(err) == 'table inbox already exists':
                ...
                sys.stderr.write('ERROR trying to create database ' +
                                 'file (message.dat). Error message: %s\n' % str(err))
                os._exit(0)
        ...
        if not shared.config.has_option('bitmessagesettings', 'identiconsuffix'):  # FIXME: Acts as a salt -- NON-SECURE as it is non-CSPRNG
        ...
        # In table inventory and objectprocessorqueue,
        # objecttype is now an integer (it was a
        # human-friendly string previously)
        item = '''SELECT value FROM settings WHERE key='version';'''
        parameters = ''
        self.cur.execute(item, parameters)
        currentVersion = int(self.cur.fetchall()[0][0])
        ...
        if currentVersion == 7:
            ...
            # Any sending messages for which we *thought*
            # that we had the pubkey must be rechecked.
        ...
        if currentVersion == 9:
            ...
            for row in queryResult:
                hsh, addressVersion = row
                address = encodeAddress(addressVersion, 1, hsh)
                item = '''UPDATE pubkeys SET address=? WHERE hash=?;'''
                parameters = (address, hsh)
                self.cur.execute(item, parameters)
        ...
        try:
            ...
            self.conn.commit()
            if transmitdata == '':
                logger.fatal('Problem: The version of SQLite you ' +
                             'have cannot store Null values. Please ' +
                             'download and install the latest revision '+
                             'of your version of Python (for example, ' +
                             'the latest Python 2.7 revision) and try again.\n')
                logger.fatal('PyBitmessage will now exit very abruptly. ' +
                             'You may now see threading errors related to ' +
                             'this abrupt exit but the problem you need ' +
                             'to solve is related to SQLite.\n\n')
                os._exit(0)
        except Exception as err:
            if str(err) == 'database or disk is full':
                logger.fatal('(While null value test) [ALERT]: ' +
                             'Your disk or data storage volume ' +
                             'is full. sqlThread will now exit.')
                shared.UISignalQueue.put(('alert', (tr._translate("MainWindow", "Disk full"),
                                         tr._translate("MainWindow", 'Alert: Your disk or data storage volume is full. Bitmessage will now exit.'), True)))
                if shared.daemon:
                    os._exit(0)
                else:
                    return
        ...
        for row in queryreturn:
            ...
            if int(value) < int(time.time()) - 86400:
                logger.info('It has been a long time since ' +
                            'the messages.dat file has been ' +
                            'vacuumed. Vacuuming now...')
                try:
                    self.cur.execute(''' VACUUM ''')
                except Exception as err:
                    if str(err) == 'database or disk is full':
                        logger.fatal('(While VACUUM) [ALERT]: ' +
                                     'Your disk or data storage ' +
                                     'volume is full. sqlThread ' +
                                     'will now exit.')
                        shared.UISignalQueue.put(('alert', (tr._translate("MainWindow", "Disk full"),
                                                 tr._translate("MainWindow", 'Alert: Your disk or data storage volume is full. Bitmessage will now exit.'), True)))
                        if shared.daemon:
        ...
        while True:
            ...
            if item == 'commit':
                try:
                    ...
                    if str(err) == 'database or disk is full':
                        logger.fatal('(While committing) [ALERT]: ' +
                                     'Your disk or data storage ' +
                                     'volume is full. sqlThread ' +
                                     'will now exit.')
                        shared.UISignalQueue.put(('alert', (tr._translate("MainWindow", "Disk full"),
                ...
                try:
                    ...
                    if str(err) == 'database or disk is full':
                        logger.fatal('(while movemessagstoprog) [ALERT]: ' +
                                     'Your disk or data storage ' +
                                     'volume is full. sqlThread ' +
                                     'will now exit.')
                        shared.UISignalQueue.put(('alert', (tr._translate("MainWindow", "Disk full"),
                        ...
                        if shared.daemon:
                            ...
                            return
                self.conn.close()
                shutil.move(shared.lookupAppdataFolder() + 'messages.dat', shared.lookupExeFolder() + 'messages.dat')
                ...
                try:
                    ...
                    if str(err) == 'database or disk is full':
                        logger.fatal('(while movemessagstoappdata) [ALERT]: ' +
                                     'Your disk or data storage ' +
                                     'volume is full. sqlThread ' +
                                     'will now exit.')
                        shared.UISignalQueue.put(('alert', (tr._translate("MainWindow", "Disk full"),
                        ...
                        if shared.daemon:
                            ...
                            return
                self.conn.close()
                shutil.move(shared.lookupExeFolder() + 'messages.dat', shared.lookupAppdataFolder() + 'messages.dat')
                ...
                try:
                    ...
                    if str(err) == 'database or disk is full':
                        logger.fatal('(while deleteandvacuume) [ALERT]: ' +
                                     'Your disk or data storage volume is ' +
                                     'full. sqlThread will now exit.')
                        shared.UISignalQueue.put(('alert', (tr._translate("MainWindow", "Disk full"),
                                                 tr._translate("MainWindow", 'Alert: Your disk or data storage volume is full. Bitmessage will now exit.'), True)))
                        if shared.daemon:
                            os._exit(0)
                ...
                try:
                    ...
                    if str(err) == 'database or disk is full':
                        logger.fatal('(while cur.execute) [ALERT]: ' +
                                     'Your disk or data storage volume is ' +
                                     'full. sqlThread will now exit.')
                        shared.UISignalQueue.put(('alert', (tr._translate("MainWindow", "Disk full"),
                                                 tr._translate("MainWindow", 'Alert: Your disk or data storage volume is full. Bitmessage will now exit.'), True)))
                        ...
                        logger.fatal('[FATAL]: Major error occurred when trying to ' +
                                     'execute a SQL statement within the sqlThread. ' +
                                     'Please tell Atheros about this error message ' +
                                     'or post it in the forum! Error occurred while ' +
                                     'trying to execute statement: "%s"  Here are the ' +
                                     'parameters; you might want to censor this ' +
                                     'data with asterisks (***) as it can contain ' +
                                     'private information: %s. Here is the actual ' +
                                     'error message thrown by the sqlThread: %s', 
