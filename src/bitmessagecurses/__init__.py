...
quit = False
menutab = 1
menu = ["Inbox", "Send", "Sent",
        "Your Identities", "Subscriptions",
            ...
            for i, item in enumerate(subscriptions[max(min(len(subscriptions) - curses.LINES + 6, subcur - 5), 0):]):
                if 6 + i < curses.LINES:
                    ...
                    if i == subcur - max(min(len(subscriptions) - curses.LINES + 6, subcur - 5), 0):  # Highlight current address
                        a = a | curses.A_REVERSE
                    if item[2] is True:  # Embolden enabled subscriptions
                        a = a | curses.A_BOLD
        ...
        if c in range(256):
            if chr(c) in '12345678':
                ...
                if menutab == 1:
                    ...
                    r, t = d.menu("What should I do with \"" + inbox[inboxcur][5] + "\" from \"" + inbox[inboxcur][3] + "\"?",
                            ...
                            if r == d.DIALOG_OK:
                                if t == "1":
                                    ...
                                    if r == d.DIALOG_OK:
                                        ...
                                        shorten = False
                                        r, t = d.checklist("Miscellaneous options", choices=[("1", "Spend time shortening the address", shorten)])
                                    ...
                                    if r == d.DIALOG_OK:
                                        if t[0] == t[1]:
                                            passphrase = t[0]
                                            r, t = d.rangebox("Number of addresses to generate", width=48, min=1, max=64, init=8)
                                            if r == d.DIALOG_OK:
                                                number = t
                                                stream = 1
                                                shorten = False
                                                ...
                                                d.scrollbox(unicode("In addition to your passphrase, be sure to remember the following numbers:\n"
                                                    ...
                                                    "  * Stream number: " + str(stream)), exit_label="Continue")
...
def sendMessage(sender="", recv="", broadcast=None, subject="", body="", reply=False):
    ...
    if not broadcast:
        ...
        if recv == "":
            ...
            d.scrollbox(unicode("You must specify an address to send the message from."), exit_label="Continue")
        else:
            ackdata = OpenSSL.rand(32)
            recv = BROADCAST_STR
            ripe = ""
...
def loadSent():
    ...
    for row in ret:
        ...
        # Load into array
        sentbox.append([tolabel, toaddr, fromlabel,
                        fromaddr, subject, statstr,
                        ackdata,
...
def run(stdscr):
    ...
    for addressInKeysFile in configSections:
        if addressInKeysFile != "bitmessagesettings":
            ...
            addresses.append([shared.config.get(addressInKeysFile, "label"),
                              isEnabled, addressInKeysFile])
            # Set address color
            if not isEnabled:
