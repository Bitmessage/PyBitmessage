...
class Blacklist(QtGui.QWidget, RetranslateMixin):
    """Instantiation of Blacklist class"""
    # pylint: disable=no-member
    def __init__(self, parent=None):
        """initiate the button attribute for blacklist and whitelist"""
    ...
    def click_pushButtonAddBlacklist(self):
        """add address and check if the address is already in the address book"""
        # pylint: disable=attribute-defined-outside-init
        self.NewBlacklistDialogInstance = AddAddressDialog(self)
        if self.NewBlacklistDialogInstance.exec_():
            if self.NewBlacklistDialogInstance.labelAddressCheck.text() == _translate(
                ...
                t = (address,)
                if (
                        BMConfigParser().get("bitmessagesettings", "blackwhitelist")
                        == "black"
                ):
                ...
                if queryreturn == []:
                    ...
                    newItem = QtGui.QTableWidgetItem(
                        unicode(
                            self.NewBlacklistDialogInstance.lineEditLabel.text().toUtf8(),
                            "utf-8"
                        )
                    )
    ...
    def tableWidgetBlacklistItemChanged(self, item):
        """update the blacklist with label and address"""
        if item.column() == 0:
    ...
    def init_blacklist_popup_menu(self, connectSignal=True):
        """initialization of blacklist popup menu"""
    ...
    def on_action_BlacklistNew(self):
        """method used for adding to blacklist using button"""
        self.click_pushButtonAddBlacklist()

    def on_action_BlacklistDelete(self):
        """method used for delete entry from blacklist"""
        currentRow = self.tableWidgetBlacklist.currentRow()
        labelAtCurrentRow = (
            self.tableWidgetBlacklist.item(currentRow, 0).text().toUtf8()
        )
    ...
    def on_action_BlacklistClipboard(self):
        """on_action_BlacklistClipboard method"""
        currentRow = self.tableWidgetBlacklist.currentRow()
        addressAtCurrentRow = self.tableWidgetBlacklist.item(currentRow, 1).text()
