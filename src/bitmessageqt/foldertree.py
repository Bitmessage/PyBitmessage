...
class AccountMixin(object):
    ...
    def defaultLabel(self):
        ...
        if self.type in (AccountMixin.NORMAL, AccountMixin.CHAN,
            ...
            try:
                retval = unicode(BMConfigParser().get(
...
class Ui_SubscriptionWidget(Ui_AddressWidget, AccountMixin):
    ...
    def setData(self, column, role, value):
        if role == QtCore.Qt.EditRole:
            ...
            if isinstance(value, QtCore.QVariant):
                label = str(value.toString().toUtf8()
                            ).decode('utf-8', 'ignore')
            else:
    ...
    def setLabel(self, label=None):
        ...
        if label is None:
            ...
            if self.type in (AccountMixin.NORMAL, AccountMixin.CHAN, AccountMixin.MAILINGLIST):
                try:
                    newLabel = unicode(BMConfigParser().get(
                        self.address, 'label'), 'utf-8', 'ignore')
                except:
                    queryreturn = sqlQuery(
                        '''select label from addressbook where address=?''',
                        self.address)
    ...
    def setData(self, role, value):
        if role == QtCore.Qt.EditRole:
            ...
            if self.type in (AccountMixin.NORMAL, AccountMixin.MAILINGLIST, AccountMixin.CHAN):
                ...
                from debug import logger
                sqlExecute(
                    '''UPDATE subscriptions set label=? WHERE address=?''', self.label, self.address)
            else:
                pass
...
class AddressBookCompleter(QtGui.QCompleter):
    ...
    def pathFromIndex(self, index):
        autoString = unicode(index.data(
            QtCore.Qt.EditRole).toString().toUtf8(), encoding="UTF-8")
