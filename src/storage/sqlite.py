...
class SqliteInventory(InventoryStorage):  # pylint: disable=too-many-ancestors
    ...
    def __init__(self):
        ...
        # of objects (like msg payloads and pubkey payloads) Does not include
        # protocol headers (the first 24 bytes of each packet).
        self._inventory = {}
        # cache for existing objects, used for quick lookups if we have an object. This is used for example whenever
        # we receive an inv message from a peer to check to see what items are new to us. We don't delete things out
        # of it; instead, the singleCleaner thread clears and refills it.
        self._objects = {}
        # Guarantees that two receiveDataThreads don't receive and process the same message concurrently (probably
        # sent by a malicious individual)
        self.lock = RLock()
        ...
        with self.lock:
            ...
            values += (
                ...
                for value in sqlQuery(
                    'SELECT objecttype, streamnumber, payload, expirestime, tag FROM inventory'
                    ' WHERE objecttype=? AND tag=?',
    ...
    def unexpired_hashes_by_stream(self, stream):
        ...
        with self.lock:
            ...
            hashes += (str(payload) for payload,
                       in sqlQuery('SELECT hash FROM inventory WHERE streamnumber=? AND expirestime>?', stream, t))
            return hashes
