        ...
        if self.radioButtonRandomAddress.isChecked():
            if self.radioButtonMostAvailable.isChecked():
                ...
                streamNumberForAddress = decodeAddress(
                    self.comboBoxExisting.currentText()
                )[2]
            queues.addressGeneratorQueue.put(
                (
                    ...
                    str(self.newaddresslabel.text().toUtf8()),
                    1,
                    "",
            ...
            if self.lineEditPassphrase.text() != self.lineEditPassphraseAgain.text():
    ...
    def __init__(self, parent=None, config=None):
        ...
        try:
            ...
            if self.address_is_chan:  # address is a chan address
                ...
                try:
                    mailingListName = config.get(self.address, "mailinglistname")
                except Exception:
                    mailingListName = ""
    ...
    def accept(self):
        ...
        if self.radioButtonBehaveNormalAddress.isChecked():
            ...
            self.config.set(
                str(self.address),
                "mailinglistname",
                str(self.lineEditMailingListName.text().toUtf8())
            )
            self.parent.setCurrentItemColor(QtGui.QColor(137, 4, 177))  # magenta
