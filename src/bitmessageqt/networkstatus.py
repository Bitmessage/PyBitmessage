...
class NetworkStatus(QtGui.QWidget, RetranslateMixin):
    ...
    def updateNetworkStatusTab(self, outbound, add, destination):
        ...
        if outbound:
            try:
                c = state.BMConnectionPool.outboundConnections[destination]
            except KeyError:
                if add:
                    return
        else:
            try:
                c = state.BMConnectionPool.inboundConnections[destination]
            except KeyError:
                try:
                    c = state.BMConnectionPool.inboundConnections[destination.host]
                except KeyError:
                    if add:
                        return
        ...
        if add:
            ...
            if not state.BMConnectionPool.inboundConnections:
