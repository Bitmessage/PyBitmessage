...
================================
"""
# pylint: disable=too-many-nested-blocks,too-many-statements,too-many-locals,no-member,global-statement,too-many-lines
...
import shared
import shutdown
from addresses import addBMIfNotPresent, decodeAddress, streamNumber
    ...
    if menutab in range(1, len(menu) + 1):
        if menutab == 1:  # Inbox
            ...
            for i, item in enumerate(inbox[max(min(len(inbox) - curses.LINES + 6, inboxcur - 5), 0):]):
                if 6 + i < curses.LINES:
                    a = 0
                    if i == inboxcur - max(min(len(inbox) - curses.LINES + 6, inboxcur - 5),
                                           0):  # Highlight current address
            ...
            for i, item in enumerate(sentbox[max(min(len(sentbox) - curses.LINES + 6, sentcur - 5), 0):]):
                if 6 + i < curses.LINES:
                    a = 0
                    if i == sentcur - max(min(len(sentbox) - curses.LINES + 6, sentcur - 5),
                                          0):  # Highlight current address
                        a = a | curses.A_REVERSE
            ...
            for i, item in enumerate(addresses[max(min(len(addresses) - curses.LINES + 6, addrcur - 5), 0):]):
                if 6 + i < curses.LINES:
                    a = 0
                    if i == addrcur - max(min(len(addresses) - curses.LINES + 6, addrcur - 5),
                                          0):  # Highlight current address
                        a = a | curses.A_REVERSE
            ...
            for i, item in enumerate(subscriptions[max(min(len(subscriptions) - curses.LINES + 6, subcur - 5), 0):]):
                if 6 + i < curses.LINES:
                    a = 0
                    if i == subcur - max(min(len(subscriptions) - curses.LINES + 6, subcur - 5),
                                         0):  # Highlight current address
                        a = a | curses.A_REVERSE
            ...
            for i, item in enumerate(addrbook[max(min(len(addrbook) - curses.LINES + 6, abookcur - 5), 0):]):
                if 6 + i < curses.LINES:
                    a = 0
                    if i == abookcur - max(min(len(addrbook) - curses.LINES + 6, abookcur - 5),
                                           0):  # Highlight current address
                        a = a | curses.A_REVERSE
                    stdscr.addstr(5 + i, 5, item[0][:34], a)
            ...
            for i, item in enumerate(blacklist[max(min(len(blacklist) - curses.LINES + 6, blackcur - 5), 0):]):
                if 7 + i < curses.LINES:
                    a = 0
                    if i == blackcur - max(min(len(blacklist) - curses.LINES + 6, blackcur - 5),
                                           0):  # Highlight current address
                        a = a | curses.A_REVERSE
                    if item[2]:  # Embolden enabled subscriptions
                        a = a | curses.A_BOLD
            ...
            stdscr.hline(7, 6, '-', 23)
            streamcount = []
            for _, stream in shared.connectedHostsList.items():
            ...
            stdscr.addstr(7, 40, "Processed " +
                          str(shared.numberOfMessagesProcessed).ljust(4) +
                          " person-to-person messages.")
            stdscr.addstr(8, 40, "Processed " +
                          str(shared.numberOfBroadcastsProcessed).ljust(4) +
                          " broadcast messages.")
                ...
                if menutab == 1:
                    ...
                    if r == d.DIALOG_OK:
                        if t == "1":  # View
                            ...
                            if not addresses[i][1]:  # pylint: disable=undefined-loop-variable
                                scrollbox(
                                    d, unicode("Sending address disabled, please either enable it or"
                    ...
                    if r == d.DIALOG_OK:
                        if t == "1":  # Create new address
                            ...
                            if r == d.DIALOG_OK:
                                if t == "1":
                                    ...
                                    if r == d.DIALOG_OK:
                                        ...
                                        shorten = False
                                        r, t = d.checklist(
                                            "Miscellaneous options", choices=[
                                                ("1", "Spend time shortening the address", 1 if shorten else 0)])
                                    ...
                                    if r == d.DIALOG_OK:
                                        if t[0] == t[1]:
                                            ...
                                            if r == d.DIALOG_OK:
                                                ...
                                                r, t = d.checklist(
                                                    "Miscellaneous options", choices=[
                                                        ("1", "Spend time shortening the address",
                                                         1 if shorten else 0)])
                                                if r == d.DIALOG_OK and "1" in t:
                                                    shorten = True
                                                scrollbox(
                            ...
                            if BMConfigParser().safeGetBoolean(a, "chan"):
                                scrollbox(
                                    d, unicode("This is a chan address. You cannot use it as a pseudo-mailing list."))
                            else:
                        ...
                        if t == "1":
                            ...
                            if r == d.DIALOG_OK and t == "I want to delete this subscription":
                                sqlExecute("DELETE FROM subscriptions WHERE label=? AND address=?",
                                           subscriptions[subcur][0], subscriptions[subcur][1])
                                shared.reloadBroadcastSendersForWhichImWatching()
                                del subscriptions[subcur]
                        elif t == "3":
                            sqlExecute("UPDATE subscriptions SET enabled=1 WHERE label=? AND address=?",
                                       subscriptions[subcur][0], subscriptions[subcur][1])
                            shared.reloadBroadcastSendersForWhichImWatching()
                            subscriptions[subcur][2] = True
                        elif t == "4":
                            sqlExecute("UPDATE subscriptions SET enabled=0 WHERE label=? AND address=?",
                                       subscriptions[subcur][0], subscriptions[subcur][1])
                            shared.reloadBroadcastSendersForWhichImWatching()
                    ...
                    if r == d.DIALOG_OK:
                        if t == "1":
                            ...
                            if r == d.DIALOG_OK and t == "I want to delete this Blacklist entry":
                                sqlExecute("DELETE FROM blacklist WHERE label=? AND address=?",
                                           blacklist[blackcur][0], blacklist[blackcur][1])
            ...
            if addr != "":
                ...
                if status != "success":
                    ...
                    if stream > 1 or stream == 0:
                        ...
                        continue
                    if not shared.connectedHostsList:
