...
class singleWorker(threading.Thread, StoppableThread):
    ...
    def run(self):
        ...
        queryreturn = sqlQuery(
            '''SELECT DISTINCT toaddress FROM
                sent WHERE (status='awaitingpubkey' AND folder='sent')''')
        for row in queryreturn:
            toAddress, = row
            toStatus,
            toAddressVersionNumber,
            toStreamNumber, toRipe = decodeAddress(
                toAddress)
        ...
        '''give some time for the GUI to start before we start on
            existing POW tasks.
        '''
        self.stop.wait(
            10)
        ...
        while state.shutdown == 0:
            ...
            if command == 'sendmessage':
                ...
                logger.error(
                    'Probable programming error: The command sent to the' +
                    'workerThread is weird. It is: %s\n' % command)

            queues.workerQueue.task_done()
        logger.info("Quitting...")

    '''This function also broadcasts out the pubkey message once it is done
    with the POW'''
    ...
    def doPOWForMyV2Pubkey(self, hash):
        ...
        for addressInKeysFile in configSections:
            if addressInKeysFile <> 'bitmessagesettings':
                ...
                hashFromThisParticularAddress=decodeAddress(addressInKeysFile)
        ...
        try:
            ...
            logger.error(
                'Error within doPOWForMyV2Pubkey. Could not read the keys' +
                'from the keys.dat file for a requested address. %s\n' % err)
            return
        ...
        target = 2 ** 64 / (defaults.networkDefaultProofOfWorkNonceTrialsPerByte * (len(payload) + 8 + defaults.networkDefaultPayloadLengthExtraBytes +
                                                                                    ((TTL * (len(payload) + 8 + defaults.networkDefaultPayloadLengthExtraBytes)) / (2 ** 16))))
    ...
    def sendOutOrStoreMyV3Pubkey(self, hash):
        ...
        """
        According to the protocol specification, the expiresTime along with the
        pubkey information is signed. But to be backwards compatible during the
        upgrade period, we shall sign not the expiresTime but rather the
        current time. There must be precisely a 28 day difference between the
        two.After the upgrade period we'll switch to signing the whole payload
        with the expiresTime time.
        """
        payload = pack('>Q', (embeddedTime))
        payload += '\x00\x00\x00\x01'  # object type: pubkey
        payload += encodeVarint(addressVersionNumber)  # Address version number
        ...
        try:
            ...
            logger.error(
                'Error within sendOutOrStoreMyV3Pubkey. Could not read the' +
                'keys from the keys.dat file for a requested address. %s\n'
    ...
    def sendOutOrStoreMyV4Pubkey(self, myAddress):
        ...
        try:
            ...
            logger.error(
                'Error within sendOutOrStoreMyV4Pubkey. Could not read the' +
                'keys from the keys.dat file for a requested address. %s\n'
                % err)
            return
        ...
        read the public keys in a pubkey message, a node must know the
        address first. We'll also tag, unencrypted, the pubkey with part of
        the hash so that nodes know which pubkey object to try to decrypt when
        they want to send a message.'''
        doubleHashOfAddressData = hashlib.sha512(hashlib.sha512(encodeVarint(
            addressVersionNumber) + encodeVarint(streamNumber) + hash).digest()
        ).digest()
        ...
        try:
            ...
            logger.error(
                'Error: Couldn\'t add the lastpubkeysendtime to the keys.dat' +
                'file. Error message: %s' % err)

    def sendBroadcast(self):
        # Reset just in case
        sqlExecute(
            '''UPDATE sent SET status='broadcastqueued' WHERE
            status = 'doingbroadcastpow' ''')
        queryreturn = sqlQuery(
            '''SELECT fromaddress, subject, message, ackdata, ttl,
            encodingtype FROM sent WHERE status=? and folder='sent' ''',
            'broadcastqueued')
        ...
        for row in queryreturn:
            ...
            if addressVersionNumber <= 1:
                logger.error(
                    'Error: In the singleWorker thread, the sendBroadcast' +
                    'function doesn\'t understand the address version.\n')
                return
            ...
            sqlExecute(
                '''UPDATE sent SET status='doingbroadcastpow' WHERE ackdata=?
                AND status='broadcastqueued' ''',
                ackdata)
            ...
            broadcaster's address.Anyone who knows the address can generate the
            private encryption key to decrypt the broadcast. This provides
            virtually no privacy; its purpose is to keep questionable and
            illegal content from flowing through the Internet connections and
            being stored on the disk of 3rd parties.'''
            if addressVersionNumber <= 3:
                privEncryptionKey = hashlib.sha512(encodeVarint(
                    addressVersionNumber) + encodeVarint(streamNumber) + ripe
                ).digest()[:32]
            else:
                privEncryptionKey = doubleHashOfAddressData[:32]
            ...
            '''Sanity check. The payload size should never be larger than 256
            KiB. There should be checks elsewhere in the code to not let the
            user try to send a message this large until we implement message
            continuation.'''
            if len(payload) > 2 ** 18:  # 256 KiB
                logger.critical(
                    'This broadcast object is too large to send. This should' +
                    'never happen. Object size: %s' % len(payload))
                continue
            ...
            logger.info(
                'sending inv (within sendBroadcast function) for object: ' +
    ...
    def sendMsg(self):
        # Reset just in case
        sqlExecute(
            '''UPDATE sent SET status='msgqueued' WHERE status IN
            ('doingpubkeypow', 'doingmsgpow')''')
        queryreturn = sqlQuery(
            '''SELECT toaddress, fromaddress, subject, message, ackdata,
            status, ttl, retrynumber, encodingtype FROM sent WHERE
            (status='msgqueued' or status='forcepow') and folder='sent' ''')
        for row in queryreturn:  # while we have a msg that needs some work
            toaddress, fromaddress, subject, message, ackdata, status, TTL,
            retryNumber, encoding = row
            ...
            if status == 'forcepow':
                ...
                the pubkey already because the user could not have overridden
                the message about the POW being too difficult without knowing
                ...
                pass
                '''If we are sending a message to ourselves or a chan then we
                won't need an entry in the pubkeys table; we can calculate the
                needed pubkey using the private keys in our keys.dat file.'''
            elif BMConfigParser().has_section(toaddress):
                sqlExecute(
                    '''UPDATE sent SET status='doingmsgpow' WHERE
                ...
                queryreturn = sqlQuery(
                    '''SELECT address FROM pubkeys WHERE
                    address=?''', toaddress)
                if queryreturn != []:
                    ...
                    used personally will already be set to yes because we'll
                    only ever have usedpersonally v4 pubkeys in the pubkeys
                    table.'''
                    sqlExecute(
                        '''UPDATE pubkeys SET usedpersonally='yes' WHERE
                        address=?''',
                        toaddress)
                else:
                    '''We don't have the needed pubkey in the pubkeys table
                    already.'''
                    if toAddressVersionNumber <= 3:
                        toTag = ''
                    else:
                    ...
                    if toaddress in state.neededPubkeys or toTag in state.neededPubkeys:
                        ...
                        queues.UISignalQueue.put(
                            ('updateSentItemStatusByToAddress', (toaddress, tr._translate(
                                "MainWindow", 'Encryption key was requested earlier.'))))
                        continue  
                        # on with the next msg on which we can do some work
                    else:
                        # We have not yet sent a request for the pubkey
                        needToRequestPubkey = True
                        '''If we are trying to send to address version >= 4 then
                        the needed pubkey might be encrypted in the inventory
                        If we have it we'll need to decrypt it and put it in
                        the pubkeys table.'''

                        '''The decryptAndCheckPubkeyPayload function expects
                        that the shared.neededPubkeys dictionary already
                        contains the toAddress and cryptor object associated
                        with the tag for this toAddress.'''
                            ...
                            for value in Inventory().by_type_and_tag(1, toTag):
                                ...
                                if shared.decryptAndCheckPubkeyPayload(
                                    ...
                                    sqlExecute(
                                        ...
                                        retrynumber=0 WHERE toaddress=? AND
                                        (status='msgqueued' or
                                ...
                                # else:
                                    ...
                                    correct tag- almost certainly because of
                                    malicious behavior or a badly programmed
                                    client. If there are any other pubkeys in
                                    our inventory with the correct
            ...
            # At this point we know that we have the necessary pubkey in the
            # pubkeys table.
            ...
            if not BMConfigParser().has_section(
                ...
                '''Let us fetch the recipient's public key out of our database.
                if the required proof of work difficulty is too hard then we'll
                # abort.'''
                queryreturn = sqlQuery(
                ...
                # if receiver is a mobile device who expects that their address
                        ...
                        queues.UISignalQueue.put(
                            ('updateSentItemStatusByAckdata',
                             (ackdata,
                              tr._translate(
                                  "MainWindow",
                                  'Problem: Destination is a mobile device who requests that the destination be included in the message but this is disallowed in your settings.  %1').arg(
                                  l10n.formatTimestamp()))))
                ...
                if toAddressVersionNumber == 2:
                        ...
                        ('updateSentItemStatusByAckdata',
                         (ackdata,
                          tr._translate(
                              ...
                              "is no required difficulty for version 2" +
                              "addresses like this."))))
                    ...
                    if status != 'forcepow':
                        if (
                            requiredAverageProofOfWorkNonceTrialsPerByte > BMConfigParser().getint(
                                'bitmessagesettings',
                                'maxacceptablenoncetrialsperbyte') and BMConfigParser().getint(
                                'bitmessagesettings',
                                'maxacceptablenoncetrialsperbyte') != 0) or (
                            requiredPayloadLengthExtraBytes > BMConfigParser().getint(
                                'bitmessagesettings',
                                'maxacceptablepayloadlengthextrabytes') and BMConfigParser().getint(
                                'bitmessagesettings',
                            ...
                            queues.UISignalQueue.put(
                                ('updateSentItemStatusByAckdata',
                                 (ackdata,
                                  tr._translate(
                                      "MainWindow",
                                      "Problem: The work demanded by the recipient (%1 and %2) is more difficult than you are willing to do. %3").arg(
                                      str(
                                          float(requiredAverageProofOfWorkNonceTrialsPerByte) /
                                          defaults.networkDefaultProofOfWorkNonceTrialsPerByte)).arg(
                                      str(
                    ...
                    queues.UISignalQueue.put(
                        ('updateSentItemStatusByAckdata',
                         (ackdata,
                          tr._translate(
                              "MainWindow",
                              'Problem: You are trying to send a message to yourself or a chan but your encryption key could not be found in the keys.dat file. Could not encrypt message. %1').arg(
                              l10n.formatTimestamp()))))
                    logger.error(
            ...
            if BMConfigParser().has_section(toaddress):
                logger.info(
                    'Not bothering to include ackdata because we are sending to ourselves or a chan.')
                fullAckPayload = ''
            ...
            target = 2 ** 64 / (requiredAverageProofOfWorkNonceTrialsPerByte * (len(encryptedPayload) + 8 +
                                                                                requiredPayloadLengthExtraBytes + ((TTL * (len(encryptedPayload) + 8 + requiredPayloadLengthExtraBytes)) / (2 ** 16))))
            logger.info(
                '(For msg message) Doing proof of work. Total required difficulty: %f. Required small message difficulty: %f.',
                float(requiredAverageProofOfWorkNonceTrialsPerByte) /
                defaults.networkDefaultProofOfWorkNonceTrialsPerByte,
                float(requiredPayloadLengthExtraBytes) /
                defaults.networkDefaultPayloadLengthExtraBytes)
            ...
            if BMConfigParser().has_section(toaddress) or not protocol.checkBitfield(
                    behaviorBitfield, protocol.BITFIELD_DOESACK):
                queues.UISignalQueue.put(
                    ('updateSentItemStatusByAckdata', (ackdata, tr._translate(
                        "MainWindow", "Message sent. Sent at %1").arg(
                        l10n.formatTimestamp()))))
            else:
                # not sending to a chan or one of my addresses
                queues.UISignalQueue.put(
                    ('updateSentItemStatusByAckdata', (ackdata, tr._translate(
                        "MainWindow", "Message sent. Waiting for acknowledgement. Sent on %1").arg(
                        l10n.formatTimestamp()))))
            logger.info(
                'Broadcasting inv for my msg(within sendmsg function):' +
                hexlify(inventoryHash))
    ...
    def requestPubKey(self, toAddress):
        ...
        if len(queryReturn) == 0:
            logger.critical(
                "BUG: Why are we requesting the pubkey for %s if there are no messages in the sent folder to that address?" %
                toAddress)
            return
        ...
        if addressVersionNumber <= 3:
            payload += ripe
            logger.info('making request for pubkey with ripe: %s',
                        hexlify(ripe))
        else:
            payload += tag
            logger.info(
