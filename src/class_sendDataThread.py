...
# Every connection to a peer has a sendDataThread
# (and also a receiveDataThread).
class sendDataThread(threading.Thread):
    ...
    def run(self):
        logger.debug('sendDataThread starting. ID: ' +
                     str(id(self)) +
                     '. Number of queues in sendDataQueues: ' +
                     str(len(shared.sendDataQueues)))
        while True:
            ...
            if deststream == self.streamNumber or deststream == 0:
                ...
                # streamNumber of this send data thread here:
                    ...
                    logger.debug('setting the stream number in the ' +
                                 'sendData thread (ID: ' + str(id(self)) +
                                 ') to ' + str(self.streamNumber))
                elif command == 'setRemoteProtocolVersion':
                    specifiedRemoteProtocolVersion = data
                    logger.debug('setting the remote node\'s protocol ' +
                                 'version in the sendDataThread (ID: ' +
                                 str(id(self)) + ') to ' +
                    ...
                    try:
                        ...
                        logger.error('Sending of data to ' +
                                     str(self.peer) +
                                     ' failed. sendDataThread thread ' +
                                     str(self) + ' ending now.')
                        break
        ...
        logger.info('sendDataThread ending. ID: ' +
                    str(id(self)) +
                    '. Number of queues in sendDataQueues: ' +
                    str(len(shared.sendDataQueues)))
