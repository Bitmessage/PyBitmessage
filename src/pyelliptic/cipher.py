...
class Cipher(object):
    ...
    def update(self, input):  # pylint: disable=redefined-builtin
        """"""
        # pylint: disable=invalid-slice-index
        i = OpenSSL.c_int(0)
        this_buffer = OpenSSL.malloc(b"", len(input) + self.cipher.get_blocksize())
        inp = OpenSSL.malloc(input, len(input))
        if OpenSSL.EVP_CipherUpdate(self.ctx, OpenSSL.byref(this_buffer),
                                    OpenSSL.byref(i), inp, len(input)) == 0:
            raise Exception("[OpenSSL] EVP_CipherUpdate FAIL ...")
        return this_buffer.raw[0:i.value]

    def final(self):
        """"""
        # pylint: disable=invalid-slice-index
        i = OpenSSL.c_int(0)
        this_buffer = OpenSSL.malloc(b"", self.cipher.get_blocksize())
        if (OpenSSL.EVP_CipherFinal_ex(self.ctx, OpenSSL.byref(buffer),
                                       OpenSSL.byref(i))) == 0:
            raise Exception("[OpenSSL] EVP_CipherFinal_ex FAIL ...")
        return this_buffer.raw[0:i.value]

    def ciphering(self, input):  # pylint: disable=redefined-builtin
        """Do update and final in one method"""
        buff = self.update(input)
        return buff + self.final()

    def __del__(self):
        if OpenSSL._hexversion > 0x10100000 and not OpenSSL._libreSSL:  # pylint: disable=protected-access
