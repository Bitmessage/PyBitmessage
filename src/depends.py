...
# Only really old versions of Python don't have sys.hexversion.
# We don't support them. The logging module was introduced in Python 2.3
if not hasattr(sys, 'hexversion') or sys.hexversion < 0x20300F0:
    sys.stdout.write('Python version: ' + sys.version)
    sys.stdout.write('PyBitmessage requires Python ' +
                     '2.7.3 or greater (but not Python 3)')
    sys.exit()
...
# We need to check hashlib for RIPEMD-160, as it
# won't be available if OpenSSL is not linked
# against or the linked OpenSSL has RIPEMD disabled.
def check_hashlib():
    if sys.hexversion < 0x020500F0:
        logger.error('The hashlib module is not ' +
                     'included in this version of Python.')
        return False
    import hashlib
    if '_hashlib' not in hashlib.__dict__:
        logger.error('The RIPEMD-160 hash algorithm is not ' +
                     'available. The hashlib module is not ' +
    ...
    try:
        ...
        logger.error('The RIPEMD-160 hash algorithm is not ' +
                     'available. The hashlib module utilizes ' +
                     'an OpenSSL library with RIPEMD disabled.')
        return False
    return True
...
def check_sqlite():
    if sys.hexversion < 0x020500F0:
        logger.error('The sqlite3 module is not ' +
                     'included in this version of Python.')
        return False
    try:
        import sqlite3
    ...
    # SQLITE version formula: https://sqlite.org/c3ref/c_source_id.html
...
def check_openssl():
    ...
    if re.match(r'linux|darwin|freebsd', SYS):
        try:
            import ctypes.util
    ...
    for path in paths:
        ...
        if openssl_hexversion < 0x90802F:
            logger.error('This OpenSSL library is too old. PyBitmessage requires ' +
                         'OpenSSL 0.9.8b or later with AES, Elliptic Curves (EC), ' +
                         'ECDH, and ECDSA enabled.')
            return False
        matches = cflags_regex.findall(compile_options)
        if len(matches) > 0:
            logger.error('This OpenSSL library is missing the ' +
                         'following required features: ' + ', '.join(matches) +
                         '. PyBitmessage requires OpenSSL 0.9.8b or ' +
                         'later with AES, Elliptic Curves (EC), ECDH, ' +
                         'and ECDSA enabled.')
            return False
        return True
    return False
...
def check_curses():
    if sys.hexversion < 0x20600F0:
        logger.error('The curses interface requires the ' +
                     'pythondialog package and the dialog utility.')
        return False
    try:
        import curses
    except ImportError:
    ...
    # The pythondialog author does not like Python2 str,
    # so we have to use Unicode for just the version
    # otherwise we get the repr form which includes
...
def check_pyqt():
    try:
        ...
        logger.error('The PyQt4 package is not available. ' +
                     'PyBitmessage requires PyQt 4.8 or ' +
    ...
    if PyQt4.QtCore.PYQT_VERSION < 0x40800:
        logger.error('This version of PyQt is too old. ' +
                     'PyBitmessage requries PyQt 4.8 or later.')
        passed = False
    if PyQt4.QtCore.QT_VERSION < 0x40700:
        logger.error('This version of Qt is too old. ' +
                     'PyBitmessage requries Qt 4.7 or later.')
        passed = False
    return passed
...
def check_dependencies(verbose=False, optional=False):
    ...
    # Python 2.7.3 is the required minimum.
    # Python 3+ is currently not supported, but it is
    # still useful to provide information
    # about our other requirements.
    logger.info('Python version: %s', sys.version)
    if sys.hexversion < 0x20703F0:
        logger.error('PyBitmessage requires Python 2.7.3 ' +
                     'or greater (but not Python 3+)')
        has_all_dependencies = False
    if sys.hexversion >= 0x3000000:
        logger.error('PyBitmessage currently does not support ' +
                     'Python 3+. Python 2.7.3 or greater is required.')
        has_all_dependencies = False
    ...
    if not has_all_dependencies:
        logger.critical('PyBitmessage cannot start. ' +
