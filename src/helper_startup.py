...
# The user may de-select Portable Mode in
# the settings if they want the config files
# to stay in the application data folder.
        ...
        if 'linux' in sys.platform:
            shared.config.set(
                'bitmessagesettings', 'minimizetotray', 'false')
                              # TODO:This isn't implmented yet and when True on
...
def isOurOperatingSystemLimitedToHavingVeryFewHalfOpenConnections():
    try:
        ...
        print("Info: we could not tell whether your OS is " +
              "limited to having very few half open connections " +
              "because we couldn't interpret the platform version. " +
              "Don't worry; we'll assume that it is not limited. " +
              "This tends to occur on Raspberry Pis. :", err)
        return False
