...
class singleWorker(threading.Thread, StoppableThread):
        ...
        for row in queryreturn:
            ...
            if toAddressVersionNumber <= 3:
                ...
                state.neededPubkeys[tag] = (
                    toAddress, highlevelcrypto.makeCryptor(hexlify(privEncryptionKey)))
    ...
    def sendOutOrStoreMyV4Pubkey(self, myAddress):
        ...
        try:
            ...
            logger.error(
                'Error: Couldn\'t add the lastpubkeysendtime to the keys.dat file. Error message: %s' % err)
    ...
    def sendBroadcast(self):
        ...
        for row in queryreturn:
            ...
            if len(payload) > 2 ** 18:  # 256 KiB
                logger.critical(
                    'This broadcast object is too large to send. This should never happen. Object size: %s' % len(payload))
                continue
            ...
            logger.info(
                'sending inv (within sendBroadcast function) for object: ' + hexlify(inventoryHash))
            queues.invQueue.put((streamNumber, inventoryHash))

            queues.UISignalQueue.put(('updateSentItemStatusByAckdata', (ackdata, tr._translate(
                "MainWindow", "Broadcast sent on %1").arg(l10n.formatTimestamp()))))
        ...
        for row in queryreturn:  # while we have a msg that needs some work
            ...
            # If we are sending a message to ourselves or a chan then we won't need an entry in the pubkeys table; we can calculate the needed pubkey using the private keys in our keys.dat file.
                ...
                if queryreturn != []:  # If we have the needed pubkey in the pubkey table already,
                    ...
                    if toAddressVersionNumber <= 3:
                        ...
                        toTag = hashlib.sha512(hashlib.sha512(encodeVarint(
                            toAddressVersionNumber)+encodeVarint(toStreamNumber)+toRipe).digest()).digest()[32:]
                    if toaddress in state.neededPubkeys or toTag in state.neededPubkeys:
                        ...
                        if toAddressVersionNumber >= 4:  # If we are trying to send to address version >= 4 then
                            ...
                            for value in Inventory().by_type_and_tag(1, toTag):
                                ...
                                if shared.decryptAndCheckPubkeyPayload(value.payload, toaddress) == 'successful':
            ...
            if not BMConfigParser().has_section(toaddress):  # if we aren't sending this to ourselves or a chan
                ...
                if shared.isBitSetWithinBitfield(behaviorBitfield, 30):
                    ...
                    if not shared.BMConfigParser().safeGetBoolean('bitmessagesettings', 'willinglysendtomobile'):
                        logger.info(
                            'The receiver is a mobile user but the sender (you) has not selected that you are willing to send to mobiles. Aborting send.')
                        queues.UISignalQueue.put(('updateSentItemStatusByAckdata', (ackdata, tr._translate(
                            "MainWindow", 'Problem: Destination is a mobile device who requests that the destination be included in the message but this is disallowed in your settings.  %1').arg(l10n.formatTimestamp()))))
                ...
                if toAddressVersionNumber == 2:
                    ...
                    if requiredPayloadLengthExtraBytes < defaults.networkDefaultPayloadLengthExtraBytes:
                        requiredPayloadLengthExtraBytes = defaults.networkDefaultPayloadLengthExtraBytes
                    logger.debug('Using averageProofOfWorkNonceTrialsPerByte: %s and payloadLengthExtraBytes: %s.' % (
                ...
                try:
                    ...
                    queues.UISignalQueue.put(('updateSentItemStatusByAckdata', (ackdata, tr._translate(
                        "MainWindow", 'Problem: You are trying to send a message to yourself or a chan but your encryption key could not be found in the keys.dat file. Could not encrypt message. %1').arg(l10n.formatTimestamp()))))
                    logger.error(
            ...
            # This hash will be checked by the receiver of the message to verify that toRipe belongs to them. This prevents a Surreptitious Forwarding Attack.
            payload += toRipe
            ...
            try:
                ...
                sqlExecute(
                    '''UPDATE sent SET status='badkey' WHERE ackdata=?''', ackdata)
                queues.UISignalQueue.put(('updateSentItemStatusByAckdata', (ackdata, tr._translate(
                    "MainWindow", 'Problem: The recipient\'s encryption key is no good. Could not encrypt message. %1').arg(l10n.formatTimestamp()))))
                continue
            ...
            logger.info('(For msg message) Doing proof of work. Total required difficulty: %f. Required small message difficulty: %f.', float(
                requiredAverageProofOfWorkNonceTrialsPerByte) / defaults.networkDefaultProofOfWorkNonceTrialsPerByte, float(requiredPayloadLengthExtraBytes) / defaults.networkDefaultPayloadLengthExtraBytes)
            ...
            if len(encryptedPayload) > 2 ** 18:  # 256 KiB
                logger.critical(
                    'This msg object is too large to send. This should never happen. Object size: %s' % len(encryptedPayload))
                continue
            ...
            if BMConfigParser().has_section(toaddress) or not protocol.checkBitfield(behaviorBitfield, protocol.BITFIELD_DOESACK):
                queues.UISignalQueue.put(('updateSentItemStatusByAckdata', (ackdata, tr._translate(
                    "MainWindow", "Message sent. Sent at %1").arg(l10n.formatTimestamp()))))
            else:
                # not sending to a chan or one of my addresses
                queues.UISignalQueue.put(('updateSentItemStatusByAckdata', (ackdata, tr._translate(
                    "MainWindow", "Message sent. Waiting for acknowledgement. Sent on %1").arg(l10n.formatTimestamp()))))
            logger.info(
                'Broadcasting inv for my msg(within sendmsg function):' + hexlify(inventoryHash))
    ...
    def requestPubKey(self, toAddress):
        ...
        if len(queryReturn) == 0:
            logger.critical(
                "BUG: Why are we requesting the pubkey for %s if there are no messages in the sent folder to that address?" % toAddress)
            return
        ...
        if addressVersionNumber <= 3:
            ...
            # Note that this is the first half of the sha512 hash.
            privEncryptionKey = hashlib.sha512(hashlib.sha512(encodeVarint(
                addressVersionNumber)+encodeVarint(streamNumber)+ripe).digest()).digest()[:32]
        ...
        queues.UISignalQueue.put(('updateSentItemStatusByToAddress', (toAddress, tr._translate(
            "MainWindow", 'Sending public key request. Waiting for reply. Requested at %1').arg(l10n.formatTimestamp()))))
